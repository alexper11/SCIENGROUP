...
try:
    ...
    gruplac_basico = pd.read_sql_query('SELECT * FROM basico', gruplac_session.bind, dtype = str)
    #gruplac_basico=gruplac_basico.drop('id',axis=1)
    gruplac_caplibros = pd.read_sql_query('SELECT * FROM caplibros', gruplac_session.bind, dtype = str)
    gruplac_caplibros=gruplac_caplibros.drop('id',axis=1)
    gruplac_integrantes = pd.read_sql_query('SELECT * FROM integrantes', gruplac_session.bind, dtype = str)
    gruplac_integrantes=gruplac_integrantes.drop('id',axis=1)
    gruplac_libros = pd.read_sql_query('SELECT * FROM libros', gruplac_session.bind, dtype = str)
    gruplac_libros=gruplac_libros.drop('id',axis=1)
    gruplac_oarticulos = pd.read_sql_query('SELECT * FROM otros_articulos', gruplac_session.bind, dtype = str)
    gruplac_oarticulos=gruplac_oarticulos.drop('id',axis=1)
    gruplac_olibros = pd.read_sql_query('SELECT * FROM otros_libros', gruplac_session.bind, dtype = str)
    gruplac_olibros=gruplac_olibros.drop('id',axis=1)
    gruplac_cdoctorado= pd.read_sql_query('SELECT * FROM curso_doctorado', gruplac_session.bind, dtype = str)
    gruplac_cdoctorado=gruplac_cdoctorado.drop('id',axis=1)
    gruplac_cmaestria= pd.read_sql_query('SELECT * FROM curso_maestria', gruplac_session.bind, dtype = str)
    gruplac_cmaestria=gruplac_cmaestria.drop('id',axis=1)
    gruplac_disenoind= pd.read_sql_query('SELECT * FROM diseno_industrial', gruplac_session.bind, dtype = str)
    gruplac_disenoind=gruplac_disenoind.drop('id',axis=1)
    gruplac_empresatec= pd.read_sql_query('SELECT * FROM empresa_tecnologica', gruplac_session.bind, dtype = str)
    gruplac_empresatec=gruplac_empresatec.drop('id',axis=1)
    gruplac_innovaempresa= pd.read_sql_query('SELECT * FROM innovacion_empresarial', gruplac_session.bind, dtype = str)
    gruplac_innovaempresa=gruplac_innovaempresa.drop('id',axis=1)
    gruplac_instituciones= pd.read_sql_query('SELECT * FROM instituciones', gruplac_session.bind, dtype = str)
    gruplac_instituciones=gruplac_instituciones.drop('id',axis=1)
    gruplac_lineas= pd.read_sql_query('SELECT * FROM lineas', gruplac_session.bind, dtype = str)
    gruplac_lineas=gruplac_lineas.drop('id',axis=1)
    gruplac_otecnologicos= pd.read_sql_query('SELECT * FROM otros_tecnologicos', gruplac_session.bind, dtype = str)
    gruplac_otecnologicos=gruplac_otecnologicos.drop('id',axis=1)
    gruplac_pdoctorado= pd.read_sql_query('SELECT * FROM programa_doctorado', gruplac_session.bind, dtype = str)
    gruplac_pdoctorado=gruplac_pdoctorado.drop('id',axis=1)
    gruplac_plantapiloto= pd.read_sql_query('SELECT * FROM planta_piloto', gruplac_session.bind, dtype = str)
    gruplac_plantapiloto=gruplac_plantapiloto.drop('id',axis=1)
    gruplac_pmaestria= pd.read_sql_query('SELECT * FROM programa_maestria', gruplac_session.bind, dtype = str)
    gruplac_pmaestria=gruplac_pmaestria.drop('id',axis=1)
    gruplac_prototipos= pd.read_sql_query('SELECT * FROM prototipos', gruplac_session.bind, dtype = str)
    gruplac_prototipos=gruplac_prototipos.drop('id',axis=1)
    gruplac_software= pd.read_sql_query('SELECT * FROM software', gruplac_session.bind, dtype = str)
    gruplac_software=gruplac_software.drop('id',axis=1)
    scopus_autores= pd.read_sql_query('SELECT * FROM autores', scopus_session.bind, dtype = str)
    scopus_autores=scopus_autores.drop('id',axis=1)
    scopus_productos=pd.read_sql_query('SELECT * FROM productos', scopus_session.bind, dtype = str)
    scopus_productos=scopus_productos.drop('id',axis=1)
    cvlac_areas=pd.read_sql_query('SELECT * FROM actuacion', cvlac_session.bind, dtype = str)
    cvlac_areas=cvlac_areas.drop('id',axis=1)
    cvlac_articulos=pd.read_sql_query('SELECT * FROM articulos', cvlac_session.bind, dtype = str)
    cvlac_articulos=cvlac_articulos.drop('id',axis=1)
    cvlac_basico=pd.read_sql_query('SELECT * FROM basico', cvlac_session.bind, dtype = str)
    #cvlac_basico=cvlac_basico.drop('id',axis=1)
    cvlac_lineas=pd.read_sql_query('SELECT * FROM investigacion', cvlac_session.bind, dtype = str)
    cvlac_lineas=cvlac_lineas.drop('id',axis=1)
    cvlac_libros=pd.read_sql_query('SELECT * FROM libros', cvlac_session.bind, dtype = str)
    cvlac_libros=cvlac_libros.drop('id',axis=1)
    cvlac_reconocimiento=pd.read_sql_query('SELECT * FROM reconocimiento', cvlac_session.bind, dtype = str)
    cvlac_reconocimiento=cvlac_reconocimiento.drop('id',axis=1)
    cvlac_caplibros=pd.read_sql_query('SELECT * FROM caplibros', cvlac_session.bind, dtype = str)
    cvlac_caplibros=cvlac_caplibros.drop('id',axis=1)
    cvlac_empresatec=pd.read_sql_query('SELECT * FROM empresa_tecnologica', cvlac_session.bind, dtype = str)
    cvlac_empresatec=cvlac_empresatec.drop('id',axis=1)
    cvlac_innovaempresa=pd.read_sql_query('SELECT * FROM innovacion_empresarial', cvlac_session.bind, dtype = str)
    cvlac_innovaempresa=cvlac_innovaempresa.drop('id',axis=1)
    cvlac_prototipos=pd.read_sql_query('SELECT * FROM prototipo', cvlac_session.bind, dtype = str)
    cvlac_prototipos=cvlac_prototipos.drop('id',axis=1)
    cvlac_software=pd.read_sql_query('SELECT * FROM software', cvlac_session.bind, dtype = str)
    cvlac_software=cvlac_software.drop('id',axis=1)
    cvlac_tecnologicos=pd.read_sql_query('SELECT * FROM tecnologicos', cvlac_session.bind, dtype = str)
    cvlac_tecnologicos=cvlac_tecnologicos.drop('id',axis=1)
    cvlac_identificadores=pd.read_sql_query('SELECT * FROM identificadores', cvlac_session.bind, dtype = str)
